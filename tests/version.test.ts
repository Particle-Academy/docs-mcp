 ...
 *
 * It is what this MCP server puts in the `serverInfo` of every `initialize`
 * response, so it is the number every connecting agent records and may gate on.
 * It said `"0.1.0"` while the package shipped as 0.2.0, and the comment beside
 * it read "for now keep static and bump in package.json" — which describes the
 * failure rather than mitigating it. Nobody bumped it.
 *
